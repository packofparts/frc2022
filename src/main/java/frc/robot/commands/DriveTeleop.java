...
public class DriveTeleop extends CommandBase {
  ...
  private void drive(double xSpeed, double ySpeed, double rotation, boolean deadZone) {
    ...
    if (rotation != 0) {
      rotation = Constants.maxTurnOutput * rotation;//ratePID.calculate(drive.getRate(), rotation*Constants.maxRate) + rotation * Constants.maxTurnOutput;
    ...
    //GYRO HOLD
      ...
      if (drive.useGyroHold) {
        ...
        if (drive.getGyroHold() != null) {
          rotation = 0;//-turnPID.calculate(drive.getAngle(), drive.getGyroHold())*Constants.maxTurnOutput; //ratePID.calculate((-turnPID.calculate(drive.getAngle(), drive.getGyroHold()))*Constants.maxRate)*Constants.maxTurnOutput;
        }
      }
    }
    ...
    if (drive.usingXboxController) {
      if (drive.getDriveJoystick().getRawButton(10)) speeds = new ChassisSpeeds(xSpeed, ySpeed, rotation);
      else speeds = ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, rotation, Rotation2d.fromDegrees(-drive.getAngle()));
    }
    else {
      if (drive.getDriveJoystickMain().getRawButton(1)) speeds = new ChassisSpeeds(xSpeed, ySpeed, rotation);
      else speeds = ChassisSpeeds.fromFieldRelativeSpeeds(xSpeed, ySpeed, rotation, Rotation2d.fromDegrees(-drive.getAngle()));
    }
    ...
    //set motor speeds
    drive.setFrontLeft(Constants.velocityPID.kSlot, ControlType.kVelocity);
    drive.setFrontRight(Constants.velocityPID.kSlot, ControlType.kVelocity);
    drive.setBackLeft(Constants.velocityPID.kSlot, ControlType.kVelocity);
    drive.setBackRight(Constants.velocityPID.kSlot, ControlType.kVelocity);
